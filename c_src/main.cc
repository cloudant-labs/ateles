...
main(int argc, char* argv[])
{
    ...
    opts.add_options("", {
        ...
        },
        {
            ...
            cxxopts::value<std::string>()->default_value("cert.pem")
        },
        {
            "k,key",
            "SSL private key to use.",
            cxxopts::value<std::string>()->default_value("key.pem")
        },
        {
            "m,max-mem",
    ...
    try {
        ...
        opts.cert = cfg["cert"].as<std::string>();
        opts.key = cfg["key"].as<std::string>();
